...
%% conv1
weights_conv1 = reshape(double(weights_conv1), conv1_patchsize, conv1_patchsize, conv1_filters);
conv1_data = zeros(hei, wid, conv1_filters);
for i = 1 : conv1_filters
    conv1_data(:,:,i) = imfilter(im_b, weights_conv1(:,:,i), 'same', 'replicate');
    conv1_data(:,:,i) = max(conv1_data(:,:,i) + double(biases_conv1(i)), 0);
end
...
for i = 1 : conv2_filters
    ...
    conv2_data(:,:,i) = max(conv2_data(:,:,i) + double(biases_conv2(i)), 0);
end
...
im_h = conv3_data(:,:) + double(biases_conv3);